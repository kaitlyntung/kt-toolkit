...
from lfads_tf2.utils import merge_chops, load_posterior_averages
...
class RTTDataset(BaseDataset):
    def load(self, fname, multiunits=False, m1_only=True, freq=0.001):
        ...
        with h5py.File(fname, 'r') as h5file:
            ...
            if multiunits:
                ...
                for chan_name, chan_spike_refs in zip(chan_names, spike_refs):
                    ...
                    chan_times = [times for times in chan_times \
                        if times.dtype == np.float]
                    spike_times.extend(chan_times)
        # Upsample the position data to 1 ms bins
        orig_time_stamps = np.squeeze(h5dict['t'])
        orig_freq = np.around(np.median(
            np.diff(orig_time_stamps)), decimals=3)
        new_time_stamps = np.around(np.arange(
            orig_time_stamps.min(), 
            orig_time_stamps.max() + orig_freq, 
            freq), decimals=3)
        ...
        def upsample_matrix(data, kind='cubic'):
            ...
            """
            interp_fn = interp1d(
                orig_time_stamps, data, kind=kind, 
    ...
    def load_lfads_output(self, lfads_dir, overlap, smooth_pwr=2, lf2=False):
        ...
        load_func = load_posterior_averages if lf2 \
            else compat.load_posterior_averages
    ...
    def calculate_onset(self, 
        ...
        for index, row in self.trial_info.iterrows():
            ...
            under_threshold = signal[trial_start:trial_peaks.iloc[0]] < signal_threshold
    ...
    def make_trial_data(self, 
        ...
        if not allow_overlap:
            assert trial_data.clock_time.duplicated().sum() == 0, \
