...
AREA2_EXAMPLE_FILE = '/snel/share/share/data/raeed_s1/'\
    'Han_20171116_COactpas_TD-2.mat'
XDS_EXAMPLE_FILE = '/snel/share/share/data/NW_Emory_Data_Sharing/' \
...
class Area2Dataset(BaseDataset):
    ...
    def get_move_onset(self,
        ...
        valid_peak = (peak_start < td.clock_time) \
            & (td.clock_time < window_end)
        valid_move = (move_start < td.clock_time) \
            & (td.clock_time < window_end)
        ...
        peak_onset_ixs = (
            under_thresh[::-1]
            .groupby(td_subset.trial_id[::-1])
            .idxmax()
        )
        # Use a simple movement threshold as backup
        onset_thresh = (
            ((trial_data[move_field] > s_thresh) & valid_move)
            .groupby(trial_data.trial_id)
        )
        ...
        logger.info(f'Move onset calculated by peak on {n_peak} trials '
...
class XDSDataset(BaseDataset):
    def load(self, fpath, sort_channels=False, fixed_channel_count=0):
        ...
        if not h5:
            ...
            feat_names = {}
            feat_names['spikes'] = [name[0] \
                for name in xds['unit_names'][0][0][0].tolist()]
            ...
            if xds['has_EMG'][0][0][0]:
                feat_names['emg'] = [name[0] \
                    for name in xds['EMG_names'][0][0][0].tolist()]
            ...
            self.spike_times = [
                f[item[0]].value for item in xds['spikes'].value
            ]
            # collect feature names
            feat_names = {}
            feat_names['spikes'] = [
        ...
        if not h5:
            trial_info_header = [
                head[0][0]
                for head in xds['trial_info_table_header'][0][0].tolist()
            ]
            ...
            for i in range(xds['trial_info_table'].shape[1]): # each trial
                ...
                for j in range(xds['trial_info_table'].shape[0]): # each field
                    if trial_info_header[j] == 'result':
                        val = to_str(
                            f[xds['trial_info_table'][j, i]].value.squeeze())
                    else:
        ...
        for signal_type, channels in self.feat_names.items():
            ...
            midx = pd.MultiIndex.from_tuples(midx_tuples, names=('signal_type', 'channel'))
            ...
            if signal_type == 'spikes':
                ...
                for times in self.spike_times:
                    ...
                    # add extra bin_width to specify the right boundary of the largest bin
                    bins = np.arange(time_frame[0], time_frame[-1] + 2 * self.bin_width, self.bin_width)
                ...
                signal_type_data = pd.DataFrame(spike_counts, index=time_frame, columns=midx)
                ...
                cols_num = cols.str.extract('(\d+)', expand=False).astype(int)
                ...
                sR = 1./(np.unique(np.round(np.diff(raw_emg_time), 8))[0]) # sampling rate 
                ...
                # notch_signal = self.apply_pl_harmonic_filter(raw_signal, sR, n_harmonics=23)
                ...
                #ax[0].set_title(self.name + ' Spectra (Raw), ' + feat_names['emg_notch'][v])
                ...
                #ax[0].set_ylabel('log PSD [V**2/Hz]')
                ax[0].legend()
                ax[0].set_title(self.name + 'EMG Spectra Comparison' + feat_names['emg_notch'][v])
                ...
                resampled_signal = scipy.signal.resample_poly(rect_signal, up=1, down=sR/1000.) #uses FIR filter like matlab function - anti aliasing

                signal_type_data = pd.DataFrame(resampled_signal, index=time_frame, columns=midx)

            elif signal_type == 'force_notch': 
                signal_type = 'raw_force'

                raw_signal = xds[signal_type][0][0]
                sR = 1./(np.unique(np.round(np.diff(raw_force_time), 8))[0]) # sampling rate 
                ...
                for n_freq, n_bw in zip(notch_cent_freq, notch_bw_freq):
                    b, a = signal.iirnotch(n_freq, n_bw, fs=1/dataset.bin_size)
                    notch_signal = signal.filtfilt(b, a, notch_signal, axis=0)
                ...
                if signal_type.startswith('kin'):
                    ...
                    if sum(intervals==interval) < kin_mat.shape[0]//2:
                        """ if more than half of the values are zero, then the signal is zero-filled
                        so apply sample and hold
                        """
                        logger.warning('Sample and hold the zero-filled values for the kin field!')
    ...
    def load_XDS_resample(self, base_path, file_name, sort_channels=False, fixed_channel_count=0):
        """
        # TODO: Eliminate this?
        Loads in an XDS MATLAB file to build a Dataset object, the 
        key component of which is a multiindexed pandas DataFrame.
        ...
        # ----- compile trial info -----
        # load the data in xds format
        trial_info_header = [head[0][0] for head in xds['trial_info_table_header'][0][0].tolist()]
        trial_info_table = xds['trial_info_table'][0][0].tolist()
            ...
            if signal_type == 'spikes':
                ...
                if signal_type.startswith('kin') or signal_type.startswith('force'):
                    ...
                    if sum(intervals==interval) < kin_mat.shape[0]//2:
                        ...
                        #     i += 1
                        # signal = kin_mat 
                        signal = self.apply_notch_filter(self.apply_resampling(signal, signal_type))
                        # pdb.set_trace()
    ...
    def get_move_onset(self,
        ...
        if type(trial_data[move_field]) is pd.DataFrame:
            ...
            trial_data[new_move_field] = trial_data[move_field].apply(lambda x: np.linalg.norm(x), axis=1)
        ...
        valid_range = (search_start < td.clock_time) \
            & (search_end > td.clock_time)
        ...
        if type(m) is pd.DataFrame:
            m = m.apply(lambda x: np.linalg(norm), axis=1)
