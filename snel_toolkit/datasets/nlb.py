...
class NLBDataset(BaseDataset):
    ...
    def load(self, fpath, split_heldout=True, skip_fields=[]):
        ...
        for mask, name in zip(masks, ["spikes", "heldout_spikes"]):
            ...
            # Bin spikes using decimal truncation and np.unique - faster than
            # np.histogram with same results
    ...
    def smooth_spk(
        ...
        # deleting and concatenating new data is much faster than overwriting, but less
        # memory efficient
        # can replace with:
        # if overwrite:
